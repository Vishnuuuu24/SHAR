...
P0B requires official UCF temporal annotations for real-data completion. P0C and P1A code-only infrastructure may continue while access is pending. P5A requires both P2C and P3C; optional Phase 4 sprints do not block it once explicitly marked `SKIPPED_OPTIONAL`.
...
| `P1B` Real manifests and evaluation views | P1A; P0B for real-data completion | Implement view builders with fixtures, then generate UCF manifests/digests, grouped validation, event-only/noisy-proxy views, and class/source-video reports when official annotations are available. | No source video crosses partitions; no test-guided tuning; fixture success is not real-data completion. | Frozen manifest digests, leakage report, view counts, quarantine report; V-D2–V-D5. | Zero leakage; all headline membership is traceable; missing headline classes explicitly block the claim. |
...
| Sprint | Prerequisites | Do | Don’t / block condition | Required artifacts and verification | DoD |
|---|---|---|---|---|---|
| `P2A` Degradations and classical restoration | P1C run-complete | Implement deterministic Gaussian, salt-and-pepper, speckle, and low-light transforms plus identity, median, Gaussian, bilateral, and NLM interfaces; verify PSNR/SSIM. | Do not claim synthetic corruptions model native UCF noise; do not cache all training corruptions. | Bitwise determinism, hand metric fixtures, method/interface matrix; V-D7 and denoiser/metric checks. | Registered levels reproduce across processes/workers; shapes/ranges/metrics pass. |
