...
| Official UCF-Crime videos + temporal annotations | Video-level train labels; temporal anomaly intervals for test | Frame-index mapping and event-only evaluation; original-resolution ROI work remains optional | Official intervals, with known incompleteness disclosed | The verified Kaggle mirror, Version 1, is approved for internal non-commercial temporal-annotation manifest work with no raw redistribution. Original-resolution media remains separately conditional on source/version/terms and ROI activation. |
...
| CUVA local package | Instruction-style video understanding records with an observed 11-category anomaly ontology | Watch-only local package; no approved roadmap role | None until source, release, ontology, and terms are registered | Not eligible for use; never merge into UCF 14-label classification. |
...
- Local-data intake observed additional unverified packages on 2026-07-24: 1,950 official-UCF MP4 files and its interval package (290 MAT files plus the text-format release); 103 UCF-Crime2Local text annotations plus split/support files; 163,957 COCO JPG files plus six JSON files; 313 AVA video files plus nine annotation/support files; and 986 CUVA MP4 files plus three JSON files. Presence is not license, source-version, completeness, mapping, or evaluation eligibility evidence.
