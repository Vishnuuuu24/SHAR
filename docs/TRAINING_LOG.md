...
| P1B-VIEW-SMOKE-001 | 2026-07-20 | P1B / fixture smoke | `COMPLETED` | Frozen membership rules → provenance-separated deterministic UCF-only views | 1×→2×→3×→4✓ / fixture-only | Event-only/noisy-proxy fixture membership and gates pass | `GOOD_ENOUGH` / `NOT_APPLICABLE` / fixture code evidence only | Superseded for real-data preparation by P1B-MANIFEST-001; still not a model result. | `results/p1b/p1b-view-smoke-dd0f2476-81037629/` |
| P1B-MANIFEST-001 | 2026-07-24 | P1B / real manifest materialization | `COMPLETED` | Verified P0A/P0B Test views → D-26 outer-Train allocation | 2 metadata-only closures / CPU | 111,308 Test source rows; 1,136,536 Train + 129,809 validation rows; zero unresolved/overlap | `GOOD_ENOUGH` / `NOT_APPLICABLE` / provenance-complete manifests, no model/metric claim | Register P1C architecture/run and B-006 path; no v3 faculty render because allocation added no visual stage. | `results/p1b/p1b-real-views-57f3a29b-3b954241/`; `results/p1b/p1b-grouped-train-validation-57f3a29b-10pct-s0/` |
