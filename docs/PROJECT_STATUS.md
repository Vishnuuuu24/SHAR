...
| Field | Current value |
|---|---|
| Project state | `READY` for P1C preregistration; all P1B real manifests/views are complete and no research run exists |
| Current phase | Phase 1 — manifest construction and frozen evaluation views |
| Active sprint | `P1C-W1` — baseline/control architecture and full-run preregistration |
| Sprint state | `BLOCKED` on the explicit P1C architecture/run inputs and B-006 MPS determinism choice. P1B is complete. Optional/conditional packages remain unactivated and do not widen this authorization. |
| Code-complete | P0A/P0B/P0C/P1A/P1B/P1C controls/D-25 faculty renderer: Yes; P2A code scaffold: implemented but B-007-blocked |
| Run-complete | No — no training or research runs exist |
| Research-complete | No |
| Last verified milestone | D-26 reaffirmed the existing 10% / seed-0 / `source_video_id` split policy. P1B then materialized 1,136,536 Train and 129,809 validation rows from 1,266,345 outer-Train inventory rows: 1,576/34 source videos, zero overlap, no Test rows read, and no model/metric claim. The automatic D-25 decision correctly emitted no v3 render because this allocation adds no visual stage to the held-out faculty examples. |
| Next concrete action | Owner registers the P1C architecture, pretrained weights/checksum, full training/selection policy, and B-006 determinism path; then run the governed full baseline controls. |

## Active scope and DoD

`P0A-W2` is closed for the active core path under [`P0A_DATA_INTAKE_ADDENDUM.md`](P0A_DATA_INTAKE_ADDENDUM.md), and [`P0B_WORK_ITEM.md`](P0B_WORK_ITEM.md) is complete. The owner-authorized annotation release permits only internal non-commercial P1B manifest work; raw annotations, media, and crops remain non-redistributable. Under D-25, a local, deterministic three-image faculty preview is allowed as presentation support and is governed by [`FACULTY_PROGRESS_VISUAL_PACK.md`](FACULTY_PROGRESS_VISUAL_PACK.md); it is not a model run, a mask result, or a research claim. P1C full runs, every real/full P2A run, P2B/P2C, checkpoint selection, downstream metrics, and research conclusions remain separately gated. `P2A-CODE-W1` is `BLOCKED` by B-007, not an implicit second active work item.
...
- P1B real Test-view closure: local-only `results/p1b/p1b-real-views-57f3a29b-3b954241/` contains 111,308 source rows, 73,500 event-only rows, 111,308 noisy-proxy rows, zero unresolved membership, and all headline classes. It contains no training/evaluation metrics and is Git-ignored because manifests expose licensed filenames.
- P1B grouped Train/validation closure: local-only `results/p1b/p1b-grouped-train-validation-57f3a29b-10pct-s0/` assigns all 1,266,345 outer-Train rows into 1,136,536 Train and 129,809 validation rows under D-26, with 1,576/34 source videos and zero overlap. Its immutable report asserts that it read no Test rows and produced no model/evaluation claim.
- D-25 faculty preview: local-only `results/faculty_progress/ucf-temporal-preview-v1/` renders the original/noise/filter/temporal stages; automatic local-only `results/faculty_progress/ucf-temporal-preview-v2-p1b/` adds membership cards proving all three examples enter both real P1B Test views. Both directories remain Git-ignored because they contain licensed video derivatives.
...
- P1B final code closure: [`p1b-code-closure-final-20260720.json`](../results/p1b/p1b-code-closure-final-20260720.json); deterministic fixture-only event/noisy views pass with frozen UCF source and unique source-frame boundaries. Its former next action (real-data materialization) is now closed by the two local P1B artifacts above.
...
- Full repository suite: 119 tests pass; compile, dependency, serialized-syntax, local-link, import, immutable-artifact, owner-placeholder, convention-approval, full-streaming P0B boundary/leakage, grouped P1B allocation, all-stochastic-family DataLoader-worker determinism, and isolated wheel checks pass.
